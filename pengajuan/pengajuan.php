...
<div class="row mt">
  <div class="col-lg-12">
    <div class="form-panel">
      <h4 class="mb">
        <span class='glyphicon glyphicon-briefcase'></span> Data Pengajuan
      ...
      <form class="form-inline" role="form">
        <table class="table table-bordered table-striped table-condensed">
          ...
          $sqlku=$mysqli->query("SELECT * from t_pengajuan order by kode_pengajuan desc");
          $no=1;
          while($data=$sqlku->fetch_assoc()){
          ?>
            <tr>
              <td><?php echo $no;?></td>
              ...
              <?php
                ...
                $ang=$mysqli->query("SELECT * from t_anggota where kode_anggota='$isi'")->fetch_assoc(); 
              ?>
              ...
              <?php 
                ...
                $oo=$mysqli->query("SELECT*from t_jenis_pinjam where kode_jenis_pinjam='$rr'")->fetch_assoc();
  ...
  <div class="col-lg-12">
    <div class="form-panel">
      <h4 class="mb"><span class='glyphicon glyphicon-briefcase'></span>Data Pengajuan
        <?php $dfl=$mysqli->query("SELECT *from t_anggota where kode_anggota='$kodeaa'")->fetch_assoc(); echo $dfl['nama_anggota']; ?><span style="float:right;">
        <input type="text" id="cari" style="width:230px;height:30px;font-size:15px;" placeholder=" cari disini...">
        <?php
        $sqlverida=$mysqli->query("SELECT * FROM t_pengajuan where kode_anggota='$kodeaa' order by kode_pengajuan desc");
        $verida=$sqlverida->fetch_assoc(); 
        $num=mysqli_num_rows($sqlverida);
        ...
        <form class="form-inline" role="form">
          <table class="table table-bordered table-striped table-condensed">
            ...
            <tbody id="fbody">
            <?php
            $rino=$mysqli->query("SELECT * FROM t_pengajuan where kode_anggota='$kodeaa' order by kode_pengajuan desc");
            $no=1;
            while($data=$rino->fetch_assoc())
            {
            ?>
              <tr>
                <td><?php echo $no;?></td>
    ...
    <div class="form-panel" style="width:50%;">
      ...
      <form action="pengajuan/proses_pengajuan.php?proses=tambah" method="post">
        <div class="form-group">
          ...
          $kdp=$mysqli->query("SELECT kode_pengajuan from t_pengajuan order by kode_pengajuan desc")->fetch_assoc();
