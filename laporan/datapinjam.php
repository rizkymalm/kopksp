...
<div class="row">
 	<div class="col-lg-12">
    	<div class="form-panel">
        	...
        	<?php
            $am=$mysqli->query("select*from t_pinjam");
			...
			<form class="form-inline" role="form">
				<table class="table table-bordered table-striped table-condensed">
			    	...
			    	<tbody>
				    <?php 
				    $sql=$mysqli->query("SELECT * from t_pinjam order by kode_pinjam desc");
				    $nomer=1;
				    while($data=$sql->fetch_assoc())
			    	{
						...
						<td>';
						$jum=mysqli_num_rows($mysqli->query("SELECT*from t_angsur where kode_pinjam='$kd_p' and kode_anggota='$kd_a'"));
						$lama=$mysqli->query("SELECT lama_angsuran from t_pinjam where kode_pinjam='$kd_p' and kode_anggota='$kd_a'")->fetch_assoc();
...
<?php 
	...
	$q=$mysqli->query("SELECT*from t_angsur where kode_pinjam='$kode_pinjam' AND kode_anggota='$kode' order by kode_angsur desc");	
?>
<div class="row">
	<div class="col-lg-12">
		<div class="form-panel">
			<h4 class="mb">
				...
				Laporan Angsuran "
				<?php
					$angg=$mysqli->query("SELECT nama_anggota from t_anggota where kode_anggota='$kode'")->fetch_assoc(); 
					echo $angg['nama_anggota'];
				?>"
				<span style="float:right;">
...
<form class="form-inline" role="form">
  <table class="table table-bordered table-striped table-condensed">
    ...
    <tbody>
    <?php $sql=$mysqli->query("SELECT * from t_pinjam where status='belum lunas' order by kode_pinjam desc");
    $nomer=1;
    while($data=$sql->fetch_assoc())
    	{
...
<form class="form-inline" role="form">
  <table class="table table-bordered table-striped table-condensed">
    ...
    <tbody>
    <?php $sql=$mysqli->query("SELECT * from t_pinjam where status='belum lunas'");
    $nomer=1;
    while($data=$sql->fetch_assoc())
    	{
    		$a=$data['tgl_tempo'];$dat=date("Y-m-d");
