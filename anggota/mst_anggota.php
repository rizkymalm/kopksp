...
<div class="row mt">
 ...
 <div class="form-panel" style="border-left:5px solid#d9534f;border-right:5px solid#d9534f;">
    <p><a class="btn btn-primary" href="index.php?pilih=1.2&aksi=aktif"><i class="glyphicon glyphicon-sd-video"></i> Aktif</a> <a class="btn btn-primary" href="index.php?pilih=1.2&aksi=keluar"><i class="glyphicon glyphicon-subtitles"></i> Nonaktif</a> <a href="?pilih=1.2&aksi=tambah" class="btn btn-primary"><span class='glyphicon glyphicon-plus'></span> Tambah Anggota</a></p>
  </div>
</div>
</div>
<div class="row">
...
<?php
		...
		$query=$mysqli->query("SELECT * FROM t_jenis_simpan WHERE nama_simpanan='pokok'");
...
<div class="row mt">
  <div class="col-lg-12">
    ...
    <h4 class="mb"><span class='glyphicon glyphicon-briefcase'></span> Tambah Data Anggota</h4>
...
<div class="row mt">
  <div class="col-lg-12">
    <div class="form-panel" style="width:50%;">
    <h4 class="mb"><span class='glyphicon glyphicon-briefcase'></span>Ubah Data Anggota</h4>
...
<div class="row">
  <div class="col-lg-12">
    <div class="form-panel">
      ...
      <?php
        $am=$mysqli->query("SELECT *from t_anggota where status='aktif'");
        $jum=mysqli_num_rows($am);
        echo'<kbd style="background-color:#d9534f;">'.$jum.'</kbd>';
      ?>
      <span style="float:right;">
      ...
      <form class="form-inline" role="form">
        <table class="table table-bordered table-striped table-condensed">
          ...
          <tbody id="fbody">
          <?php
            $query=$mysqli->query("SELECT * FROM t_anggota where status='aktif'");
            $no=1;
            while($data=$query->fetch_assoc()){
          ?>
            <tr>
...
<div class="row">
    <div class="col-lg-12">
      ...
      <?php
          $am=$mysqli->query("SELECT *from t_anggota where status='keluar'");
          $jum=mysqli_num_rows($am);
      echo'<kbd style="background-color:#d9534f;">'.$jum.'</kbd>';?>
      <span style="float:right;">
      <input type="text" id="cari" style="width:230px;height:30px;font-size:15px;" placeholder=" cari disini...">
        ...
        <form class="form-inline" role="form">
          <table class="table table-bordered table-striped table-condensed">
            ...
            <tbody id="fbody">
              <?php
                $query=$mysqli->query("SELECT * FROM t_anggota where status='keluar'");
                $no=1;
                while($data=$query->fetch_assoc()){
              ?>
              <tr>
