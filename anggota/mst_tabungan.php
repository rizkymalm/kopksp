...
if(empty($aksi))
{
  $ghu=$mysqli->query("SELECT * FROM t_tabungan");
  $no=1;
  while($dataku=$ghu->fetch_assoc())
  {
    ...
    if($tempo==$tang)
    {
      ...
      $tol=$mysqli->query("UPDATE t_tabungan set tgl_mulai='$tang',besar_tabungan='$total' where kode_tabungan='$kode_tab'");
    }
    else
    {    
    }
  ...
  <div class="col-lg-12">
    <div class="form-panel">
      <h4 class="mb"><span class='glyphicon glyphicon-briefcase'></span> Data Tabungan
        <?php
          $am=$mysqli->query("select*from t_tabungan");
      ...
      <form class="form-inline" role="form">
        <table class="table table-bordered table-striped table-condensed">
          ...
          <tbody id="fbody">
          <?php
          	$query=$mysqli->query("SELECT * FROM t_tabungan order by kode_tabungan desc");
          	$no=1;
          ...
          ?>
            <tr>
              ...
              <td><?php echo $d=$data['kode_anggota']?></td>
              <?php
                $d=$data['kode_anggota'];
                $f=$mysqli->query("SELECT nama_anggota from t_anggota where Kode_anggota='$d'")->fetch_assoc(); 
                $rty=$mysqli->query("SELECT sum(besar_simpanan) as total_asli from t_simpan where Kode_anggota='$d'")->fetch_assoc(); 
              ...
              <td>
                <a class="btn btn-danger btn-xs" href="index.php?pilih=1.3&aksi=viewambil&kode_tabungan=<?php echo $data['kode_tabungan'];?>&kode_anggota=<?php echo $d;?>">
                  <i class="fa fa-dollar"></i> Ambil Uang
                </a>
              </td>
            </tr> 
            <?php
            ...
            <tr class="info"><td colspan="5" align="center">Total</td>
              <td colspan="2">
                <?php 
                $bu=$mysqli->query("SELECT sum(besar_tabungan) as besar_tab from t_tabungan")->fetch_assoc();
                ?>
                Rp. <?php echo number_format($bu['besar_tab']); ?>
              </td>
            </tr>
          </tbody>
        </table>
  ...
  <div class="col-lg-12">
    <div class="form-panel" style="width:50%;">
      ...
      <form action="anggota/proses_tabungan.php" method="get">
        ...
        <div class="form-group">
          <label>Saldo</label>
          <?php $rtyu=$mysqli->query("SELECT *FROM t_tabungan where kode_tabungan='$lo' and kode_anggota='$luy'")->fetch_assoc(); ?>
...
<?php 
  ...
  $lo=$_GET['kode_tabungan'];$luy=$_GET['kode_anggota'];
  $nama=$mysqli->query("SELECT nama_anggota from t_anggota where Kode_anggota='$luy'")->fetch_assoc(); 
?>
<div class="row mt">
  <div class="col-lg-12">
    <div class="form-panel">
      <h4 class="mb">
        <span class='glyphicon glyphicon-briefcase'></span> Data Pengambilan Uang "<?php echo $nama['nama_anggota']; ?>" 
        <span style="float:right;"></span>
      </h4>
      <form action="anggota/proses_tabungan.php" method="get">
        <input type="hidden" value="<?php echo $lo; ?>" name="kode_tabungan"> <input type="hidden" value="<?php echo $luy; ?>" name="kode_anggota"> 
        <?php 
          $rtyu=$mysqli->query("SELECT *FROM t_tabungan where kode_tabungan='$lo' and kode_anggota='$luy'")->fetch_assoc();
        ?>
      ...
      <form class="form-inline" role="form">
        <table class="table table-bordered table-striped table-condensed">
          ...
          <tbody>
          <?php
            $query=$mysqli->query("SELECT * FROM t_pengambilan where kode_anggota='$luy' and kode_tabungan='$lo' order by kode_ambil desc");
            $no=1;
            while($data=$query->fetch_assoc()){
              $qtabung = $mysqli->query("SELECT * FROM t_tabungan WHERE kode_tabungan='$data[kode_tabungan]'");
              $databung = $qtabung->fetch_assoc();
          ?>
            <tr>
              <td><?php echo $no;?></td>
              ...
              <?php
                ...
                $f=$mysqli->query("SELECT nama_anggota from t_anggota where Kode_anggota='$d'")->fetch_assoc();
                $rty=$mysqli->query("SELECT sum(besar_simpanan) as total_asli from t_simpan where Kode_anggota='$d'")->fetch_assoc();
                $inves=$databung['besar_tabungan'] - $rty['total_asli'];
              ?>
              <td><?php echo $f['nama_anggota'];?></td>
              <td><?php echo $data['kode_tabungan'];?></td>
            ...
            ?>
            <tr  class="info">
              <td colspan="5"a align="center">Total</td>
              <td colspan="2">
                <?php 
                  $bu=$mysqli->query("SELECT sum(besar_ambil) as besar_ambil from t_pengambilan where kode_anggota='$luy'")->fetch_assoc();
                ?>
                Rp. <?php echo number_format($bu['besar_ambil']); ?>
              </td>
            </tr>
          </tbody>
        </table>
