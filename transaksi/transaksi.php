		...
		<div class="form-panel">
			...
			<form class="form-inline" role="form">
				<table class="table table-bordered table-striped table-condensed">
				    ...
				    <tbody>
					<?php
					$query=$mysqli->query("SELECT * FROM t_anggota where status='aktif' ORDER BY kode_anggota ASC");
...
<?php
	...
	$nama=$mysqli->query("SELECT *from t_jenis_simpan where kode_jenis_simpan='$kode_jenis'")->fetch_assoc();
	$qubah=$mysqli->query("SELECT * FROM t_anggota WHERE kode_anggota='$kode'");
	$data2=$qubah->fetch_assoc();
?>
<div class="row mt">
    <div class="col-lg-12">
        <div class="form-panel" style="width:50%;">
    		...
    		<form action="transaksi/proses_transaksi.php?pros=simpan" method="post" id="form" name="mainform" onSubmit="validasiSimpan()">
		        ...
		        <div class="form-group">
		            ...
		            <input type="text" name="user_entri" class="form-control" size="54" value="<?php echo $_SESSION['kopname'];?>" readonly >
		        </div>
		        <div class="form-group">
			...
			<form action="transaksi/proses_pinjam.php" method="GET" id="form" name="frmAdd">
				...
				<div class="form-group">
				    ...
				    <select name="kode_jenis_pinjam" class="form-control" id="kode_jenis_pinjam" onChange="show3(this.value)" class="required" title="Jenis Pinjaman harus diisi">
				    	<?php
					    	$q=$mysqli->query("SELECT * FROM t_jenis_pinjam");
				...
				<button class="btn btn-danger" name="pinjam"><span class='glyphicon glyphicon-edit'></span> Pinjam</button>
			</form>
		</div>
	</div>
</div>
...
<?php
	...
	$jio=$mysqli->query("SELECT * from t_pinjam where kode_pinjam='$kodep' ")->fetch_assoc();
	$qubah=$mysqli->query("SELECT * FROM t_anggota WHERE kode_anggota='$kode'");
	$data2=$qubah->fetch_assoc();
?>
<div class="row mt">
     <div class="col-lg-12">
        <div class="form-panel" style="width:50%;">
			...
			<form action="transaksi/proses_transaksi.php?pros=angsur" method="post" id="form" name="frmAdd">
				...
				<?php 	
				$kk=$mysqli->query("SELECT * from t_pinjam where kode_pinjam='$kodep' and kode_anggota='$kode'")->fetch_assoc();
				$tempo=$kk['tgl_tempo'];$dat=date("Y-m-d");
				if($dat>$tempo)
				{
				...
				<label for="user_entri">User Entri</label>
					 <input type="text" class="form-control" name="u_entry" size="54" value="<?php echo $_SESSION['kopname'];?>" readonly>
				</div>
				<div class="form-group">
				<label for="tgl_entri">Tanggal Angsur</label>
					 <input type="text" class="form-control" name="tgl_entri" size="54" value="<?php echo date("Y-m-d");?>" readonly/>
				</div>
				<button class="btn btn-danger" name="angsur"><span class='glyphicon glyphicon-share'></span> Angsur</button>
			</form>
		</div>
	</div>
</div>
...
<?php
	...
	$kode=$_GET['kode_anggota'];
	$anggota=$mysqli->query("SELECT nama_anggota from t_anggota where kode_anggota='$kode'")->fetch_assoc(); 
?>
<div class="row mt">
	<div class="col-lg-12">
		<div class="form-panel">
			<h4 class="mb">
				<span class='glyphicon glyphicon-briefcase'></span> Transaksi <?php echo $anggota['nama_anggota'];?>
		        <?php
		        $am=$mysqli->query("select*from t_pinjam where kode_anggota='$kode'");
		        $jum=mysqli_num_rows($am);
		        echo'<kbd style="background-color:#d9534f;">'.$jum.'</kbd>';?>
		        <span style="float:right;">
		        <?php 
		        $df=$mysqli->query("SELECT * FROM t_pinjam where kode_anggota='$kode' order by kode_pinjam desc");
		        $qdf=$df->fetch_assoc();
		        $op=mysqli_num_rows($df);
		        if($qdf['status']=='belum lunas')
		        {
			...
			<form class="form-inline" role="form">
				<table class="table table-bordered table-striped table-condensed">
			    	...
			    	<tbody>
				    <?php 
				    $sql=$mysqli->query("SELECT * from t_pinjam where kode_anggota='$kode' order by kode_pinjam desc");
				    $nomer=1;
				    while($data=$sql->fetch_assoc())
			    	{
						...
						$dfo=mysqli_num_rows($mysqli->query("SELECT * from t_angsur where kode_pinjam='$kd_p' and kode_anggota='$kode'"));
...
<?php
	...
	$kode=$_GET['kode_anggota'];
	$q=$mysqli->query("SELECT *from t_anggota where kode_anggota='$kode'");
	$ang=$q->fetch_assoc(); 
?>
	...
	<div class="col-lg-12">
		<div class="form-panel">
			<h4 class="mb">
	            ...
	            <span style="float:right;">
				<?php
					$rino=$mysqli->query("SELECT*FROM t_jenis_simpan");
					$no=1;
					while($verida=$rino->fetch_assoc())
					{ 
						if($verida['nama_simpanan']=='wajib')
						{
						 	$baru=$mysqli->query("SELECT * FROM t_simpan where kode_anggota='$kode' and jenis_simpan='wajib' order by kode_simpan desc");
						 	$databaru=$baru->fetch_assoc();
						 	$numrow=mysqli_num_rows($baru);
						 	$data=$databaru['tgl_mulai'];
						 	$now=date("Y-m-d");
						 	if($data==$now)
						 	{
				...
				?>
					<a href="laporan/print_show_simpanan.php?kode=<?php echo $ang['kode_anggota'];?>" target="_blank" class="btn btn-primary">
			...
			<form class="form-inline" role="form">
				<table class="table table-bordered table-striped table-condensed">
					...
					<?php
					$query = $mysqli->query("SELECT * from t_simpan where kode_anggota='$kode'order by kode_simpan desc");
					echo '<tbody>';	
					$no=1;
					while($data=$query->fetch_assoc()){
					?>
					...
					<tr class="info"><td colspan="3" align="center">Total</td>
						<td>
						  	...
						  	?>
						  	Rp. <?php  echo Rp($bu['besar_simpan']); ?>
						</td>
					</tr>
					</tbody>
				</table>
